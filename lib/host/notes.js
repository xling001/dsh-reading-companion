...
//#region 回收站（v1.55）
//
// 「删除」**不动文件**：在文件尾追加一条 `<!-- drc-note:deleted id=… -->`；
// 「恢复」同理追加 `<!-- drc-note:restored id=… -->`。**每个 id 的状态 = 最后一条相关标记**
// （见 `parseTrashState`）—— 于是删除与恢复都是**纯追加**，重复点也幂等。
//
// ⚠️ 为什么不做成"就地删掉那一块"：`notes.md` 是**读者的文件**，他可能同时在 Obsidian
// 里编辑（见本文件开头对 `O_APPEND` 的说明）。追加不读文件，所以**永远不会吃掉他正在
// 打的字**。**只有「确认删除 / 清空」才读-改-写** —— 那一步是 `removeNotes`（纯函数）
// 加 `library.js` 里的"先备份 + 写前核对 mtime/大小"。

/** 采集两类回收站标记。删除与恢复各一条，**最后一条赢**。 */
const TRASH_MARKER_RE = /<!--\s*drc-note:(deleted|restored)\s+([^>]*?)-->/g

/**
 * 渲染一条删除 / 恢复标记（与笔记块的锚点用同一套 `key=value` 写法）。
 *
 * @param {string} id 笔记 id
 * @param {'deleted'|'restored'} kind 进回收站 / 从回收站恢复
 * @param {string} [at] 时间戳
 * @returns {string}
 */
export function renderTrashMarker(id, kind, at = new Date().toISOString()) {
  const name = kind === 'restored' ? 'restored' : 'deleted'
  return `<!-- drc-note:${name} ${renderAttrs({ id, at })} -->`
}

/**
 * 扫一遍全文，算出每条笔记当前在不在回收站里。
 *
 * **最后一条赢**：同一 id 的标记按**出现顺序**应用（`deleted` → `restored` → `deleted`
 * 的结果是"在回收站里"），所以重复删除 / 重复恢复都是幂等的，不需要去重。
 *
 * @param {string} markdown 笔记文件全文
 * @returns {Map<string, boolean>} id → 是否在回收站里（true = 已删除）
 */
export function parseTrashState(markdown) {
  const text = typeof markdown === 'string' ? markdown : ''
  const events = []
  const re = new RegExp(TRASH_MARKER_RE.source, 'g')
  let match = re.exec(text)
  while (match !== null) {
    const id = parseAttrs(match[2]).id
    if (typeof id === 'string' && id !== '') {
      events.push({ at: match.index, id, deleted: match[1] === 'deleted' })
    }
    match = re.exec(text)
  }
  events.sort((a, b) => a.at - b.at)
  const state = new Map()
  for (const event of events) state.set(event.id, event.deleted)
  return state
}
 ...
 * 追加一条删除 / 恢复标记。
 *
 * ⚠️ **纯追加**（`O_APPEND`，与 `appendNote` 同一套理由）：不读、不改既有字节，
 * 所以读者在 Obsidian 里同时编辑也不会丢东西。
 *
 ...
 */
export function appendTrashMarker(notesPath, id, kind) {
  const marker = renderTrashMarker(id, kind)
  if (!existsSync(notesPath)) {
    atomicWriteText(notesPath, `${emptyNotesHeader('这本书')}\n${marker}\n`)
    return { id, kind }
  }
...
/**
 * 从全文里**真正删掉**这些笔记的块与标记 —— 纯函数，唯一会改既有字节的一步。
 *
 * 调用方（`library.js` 的 `purgeNotes`）负责**先备份、再核对文件没被别人改过**。
 * 这里只做两件机械的事：① 按 `splitNoteBlocks` 给出的**原样块**精确移除整块；
 * ② 移除这些 id 自己的删除 / 恢复标记（否则标记会变成孤儿，越攒越多）。
 *
 * @param {string} markdown 文件全文
 * @param {string[]} ids 要彻底删掉的笔记 id
 * @returns {{ markdown: string, removed: number }}
 */
export function removeNotes(markdown, ids) {
  const text = typeof markdown === 'string' ? markdown : ''
  const target = new Set((Array.isArray(ids) ? ids : []).filter((id) => typeof id === 'string' && id !== ''))
  if (target.size === 0) return { markdown: text, removed: 0 }

  let out = text
  let removed = 0
  for (const block of splitNoteBlocks(text)) {
    if (block.id === '' || !target.has(block.id)) continue
    out = out.replace(block.raw, '')
    removed += 1
  }

  out = out.split('\n').filter((line) => {
    const matched = /<!--\s*drc-note:(?:deleted|restored)\s+([^>]*?)-->/.exec(line)
    if (matched === null) return true
    const id = parseAttrs(matched[1]).id
    return target.has(id) !== true
  }).join('\n')

  // 只收掉"因此多出来的"连续空行；其它内容逐字不动。
  out = out.replace(/\n{3,}/g, '\n\n')
...
export function readNotes(notesPath, title = '这本书') {
  ...
  // ⚠️ `trashed` 由**文件尾的标记**决定（v1.55，见 `parseTrashState`）。
  // 手写块没有 id，所以它永远不在回收站里 —— 回收站管的是"插件记下的笔记"。
  const trash = parseTrashState(markdown)
  const notes = parseNotes(markdown).map((note) => ({
    ...note,
    trashed: typeof note.id === 'string' && note.id !== '' && trash.get(note.id) === true,
  }))
  return { exists, markdown, notes }
}
