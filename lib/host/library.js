...
import { appendNote, deleteDraft, emptyNotesHeader, listDrafts, paginateNotes, readNotes, upsertDraft } from './notes.js'
import {
  backgroundGap,
  mergeBackground,
  parseBackground,
...
export function createLibrary(options) {
  ...
  function exportBook(bookId, options = {}) {
    ...
    const notesSource = readTextIfExists(artifactPath(bookId, 'notes.md'))
  ...
  function sampleChapters(bookId, fromIndex, toIndex, options = {}) {
    ...
    // 下限默认 **150**（v2.0.2，读者选定；v1.25 曾是 100）。v1.24 曾把它降到 60
    // （那时它是"按章长比例"那个公式的封底），v1.25 随 `lengthRatio` 一起回到旧形状。
    ...
    const minPerChapter = options.minPerChapter ?? 150
    const maxPerChapter = options.maxPerChapter ?? 600
  ...
  function notesPage(bookId, options = {}) {
    const meta = findBook(bookId)
    if (meta === undefined) throw new Error(`BOOK_NOT_FOUND: ${bookId}`)
    return paginateNotes(readNotes(notesPathFor(bookId), meta.title).notes, options)
  }
  ...
  function notesForChapter(bookId, chapterIndex) {
    ...
    return all.filter((note) => note.chapterIndex === chapterIndex).reverse()
  }
