...
`
    ...
    const NoteList = memo(function NoteList(props) {
      const { notes, total, loading, page, pageCount, hasPrev, hasNext, onPrev, onNext, onJumpToChapter } = props
      /**
       * 已展开全文的那些笔记 id。
       *
        ...
        : notes.length === 0
          ...
          : h(
              ...
              ...notes.map((note) => h(
                ...
                // 「回到原文」：这条笔记的章号是**现成的**（`<!-- drc-note:begin … chapter=N -->`
                // 里就有），所以从回顾列表跳回正文只差一个回调。回顾旧摘抄时最想要的
                // 就是"回那一章看看上下文"——而这正是从前做不到的一步。
      ...
      return h(
        ...
        // 标签显示的是**总数**而不是本页条数：分页是实现细节，读者要知道
        // 自己一共记了多少条。
        h('div', { className: 'drc-label' }, `已落盘的笔记（${total}）`),
        body,
        ...
        h(
          ...
          drafts.length === 0
            ...
            : h(
                'div',
                { className: 'drc-section' },
                h('div', { className: 'drc-label' }, `未落盘的草稿（${drafts.length}）`),
                h(
                  'div',
                  { className: 'drc-chips' },
                  ...drafts.map((draft) => h(
                    'button',
                    {
                      key: draft.draftId,
                      type: 'button',
                      className: draft.draftId === active?.draftId ? 'drc-chip drc-chip-on' : 'drc-chip',
                      onClick: () => applyActive(draft),
                    },
                    (draft.excerpt || draft.thought || '（空草稿）').slice(0, 16),
                  )),
                ),
              ),
          ...
          h(
            ...
            h(
              'div',
              { className: 'drc-row' },
              h('button', { type: 'button', className: 'drc-btn', disabled: busy, onClick: save }, '保存草稿'),
              h(
                'button',
                {
                  type: 'button',
              ...
              active === null
                ? null
                : h('button', { type: 'button', className: 'drc-btn', disabled: busy, onClick: discard }, '丢弃草稿'),
              // 「导出」**只在设置页留一个入口**（读者裁定）：两个界面各放一颗**
              // 一模一样的**按钮，只会让人怀疑"它们导到的是不是同一个地方"。
              // 所以这里给一句指路的提示，而不是第二颗按钮。
              h(
                'span',
                { className: 'drc-inline-label' },
    ...
    function CompanionView(props) {
       ...
       * **解锁要二次确认**（`window.confirm`）；**收回一键即时** —— 收紧安全不需要摩擦。
       *
      ...
      const saveFinished = useCallback((finished) => {
        ...
        if (finished === true) {
          const okToUnlock = window.confirm(
            '解锁后，陪读 AI 可以读这本书的原文（你让它翻哪一章，它就翻哪一章）。\n\n'
            + '⚠️ 只影响这一本书。而且**不影响每轮自动投喂的内容** ——\n'
            + '它平时仍然只看得到「本章全文 + 上一章结尾 + 背景认识」，解锁只是"你问，它才读得到"。\n\n'
            + '确认这本书你已经读完了吗？',
          )
        ...
        h(
          ...
          h(
            ...
            gate === null
              ...
              : h(
                  'div',
                  { style: { marginTop: 8, padding: '8px 10px', border: '1px solid #d9a441', borderRadius: 6 } },
                  h(
                    'div',
                    { className: 'drc-item-sub' },
            ...
            h(
              ...
              h(
                'button',
                {
                  ...
                  className: 'drc-btn',
                  disabled: gateSaving || background === null,
                  title: background?.finished === true
                    ? '收回解锁：这本书的原文重新对 AI 关闭（立即生效，不需要确认）'
                    : '解锁：这本书的原文对 AI 可见（会再问你一次）',
                  onClick: () => saveFinished(background?.finished !== true),
                },
                background?.finished === true ? '收回解锁' : '标记为已读完',
              ),
            ),
          ),
          // ---- 导出 ----------------------------------------------------------
