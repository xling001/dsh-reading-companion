...
/* 分段控件里「当前生效」的那一档。只用边框色太弱 —— 三选一的控件必须让人
   ...
   见 design-v1 的 v1.49 §313）—— 所以这里一律用「」引词。 */
button.drc-btn-on {
...
.drc-tab-num { margin-left: 2px; font-variant-numeric: tabular-nums; opacity: .75; }
.drc-tab-num-zero { opacity: .4; }
...
/* ⚠️ 水平内边距**必须是 0**（v1.67 统一左基线）：列表项的左边界要落在 .drc-section
   ...
   v1.67 我又违反了一次）—— 引词一律用「」。 */
.drc-note-item { border-bottom: 1px solid var(--dsw-border-subtle, rgba(128,128,128,.2)); padding: 8px 0; }
...
`
    ...
    function NotesView(props) {
       ...
       *
       * ⚠️ 它必须定义在 `setTrashed` / `runPurge` **之前**（`const` 不能先用后定义，
       * 否则渲染那一帧直接 TDZ 报错）—— 所以它紧挨着 `applyPage`，而不是挪到
       * `loadPage` 旁边。
       */
      const reloadNotesPage = useCallback(() => {
        const before = cursors[cursors.length - 1]
        return callApi(`/books/${book.bookId}/notes?limit=${NOTES_PAGE_SIZE}`
          + (before === null || before === undefined ? '' : `&before=${encodeURIComponent(before)}`))
          .then((data) => {
            if (data.reset === true) setCursors([null])
            applyPage(data)
          })
      }, [book.bookId, cursors, applyPage])
      ...
      const setTrashed = useCallback((note, kind) => {
        ...
        call
          ...
          .catch((error) => setNotice({ kind: 'error', text: describeError(error) }))
      }, [book.bookId, reloadNotesPage, loadTrash])
      ...
      const discardDraft = useCallback((target) => {
        ...
        setBusy(true)
        callApi(`/books/${book.bookId}/drafts/${target.draftId}`, { method: 'DELETE' })
          .then(() => {
