...
- **笔记区三个分页选项卡**：`笔记 / 草稿 / 回收站` —— 在用、没写完、弃掉，各有多少条一眼看清。
- **删除笔记有了回收站**：删除只是**在文件尾追加一条标记**（可一键恢复，**不动你已有的字**）；只有「确认删除 / 清空」才真正改写 `notes.md`，而那一步会**先备份**，并且**在文件正被你编辑时中止**。
- **编辑区只管"存 / 落盘"**：`写入笔记`（主）+ `保存草稿`；对"已经存在的那条"做的处理（`丢弃草稿` / `删除本条` / `恢复`）都放在列表里 —— 与「删除笔记」同一逻辑。
- **二级确认统一成一套**：解锁「已读完」、跳读闸、彻底删除 / 清空共用同一种**确认条**（不再弹浏览器原生确认框）；**收紧安全 / 可逆的动作仍然一键即时**。
- **补齐更省**：提示词的固定说明移到最前（跨批逐字相同，可被前缀缓存复用），整段 1359 → 1145 字符。
