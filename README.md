...
**DeepSeek Harness（DSH）的本地 TXT 阅读器 + 不剧透的 AI 陪读插件。** 把一本书装进右侧栏，让「读」和「聊」在同一屏发生；陪读 AI 只能看到**你已经读到的部分**，摘抄与它逐渐形成的理解都落在你自己的磁盘上，可以直接导出进 Obsidian 之类的笔记库。

- 运行形态：DSH 插件（Cordis bundle），**零运行时依赖、零构建步骤**——`lib/` 就是源码
- 阅读输入：本地 **TXT**（自动探测编码、自动切章；无标题时降级为固定块）
- AI 视野：**本章全文** + **上一章结尾** + 随进度**增量补齐**的「背景认识」；**不含你未读的部分**
- 数据位置：原书 / 笔记 / AI 的理解**全在本地**，无账号、无云、无书源
...
- **目录按卷折叠**：长篇的目录不再一次铺开 —— 默认只展开你正在读的那一卷（当前卷也能点标题收起），输入筛选词时全部展开。
- **正文页多了一行「本章你记过 N 条」**：点开是每条笔记的一行摘要与「**跳到这一段**」，点它回到原文那一段；长文仍在笔记页读（那里可「展开全文」）。
- **读完一本书可以解锁它**：在面板里声明「已读完」之后，**那一本**的原文对陪读 AI 放开（"你问，它才读得到"）；**不影响每轮自动投喂的内容**，界面常驻显示，可一键收回。
- **「前文脉络」更精炼**：每条上限 80 → 60 字，并明确"已经写过的章号范围不要再写一遍"。
- **面板整理**：「讨论历史」移到最末；「导出背景与全部笔记」收敛到设置页一个入口（笔记页原地留指路提示）。
...
| **导出** | 笔记**增量追加**（不覆盖你在笔记库写的批注与双链）、背景**整份快照**、「压缩前」**一代一个文件**；文件名自带书名、每本书一个文件夹；拒绝写进没有标记的陌生文件 | 笔记页 |
| **书友设定 / 讨论历史** | `persona.md` 管口吻与关注点（与守则一起生效，**不覆盖**硬底线）；讨论时间线保存在书目录里，可回顾 | 设置页 |
| **可选：为情节生成插图** | 不改插件——在会话里让陪读 AI 画。**情节它本来就有**（本章全文在手），**长相它没有**（外观建议写进 `persona.md`）。详见 [附](#附给某个情节生成一张插图可选) | 会话 |
...
以及落盘用的「**写入笔记**」；**导出在「设置」页**（那一页还能记住导出目录）。换笔记存放位置也在这一页。
...
- **情节它本来就有。** 发笔记时**当前章全文**已经在它的上下文里；要画**更早的章**，加一句定位或贴两句原文。
- **长相它没有。** 小说很少写发色、服饰、年龄。**一次锁定、之后复用**：写「书友设定」（`persona.md`）
  最稳（那节按你写的原样注入，吃的不是背景那份预算）；写进 `background.md` 的人物条目也行，
  但**没有章号的条目在预算不够时最先被丢**。
- **别说"你去读人物卡"。** 那张卡是**给你看的视图**，它看不到；它的人物信息来自 `background.md`。
  想让它抓住谁，**直接点名字**。
...
| `sample.minPerChapter` | number | `150` | 默认形态下这是"均分额度的下限"，**同时决定一批能吞多少章**：`一批章数 ≈ budgetChars / minPerChapter` |
| `sample.maxPerChapter` | number | `600` | 单章上限 |
...
| `sample.recentMinPerChapter` | number | `300` | 只给 `recent` 路径用的每章下限（比 `minPerChapter` 厚：那条路要的是"能聊这一章"，不是"不致迷路"） |
...
docs/                       # design-v1（追加式修订块）/ manual-testing / publishing / frontend-audit
```
...
```bash
npm test                                  # 全部测试（Node 内置 test runner，551 项）
