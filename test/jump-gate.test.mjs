...
test('跳读闸：**手动补**时缺口超阈值不自动补，回 409 且**一次调用都没发**', async () => {
  ...
  try {
    ...
    assert.equal(res.body.estimate.all.perChapter, 150, '全量纳入的每章下限（v2.0.2：100 → 150）')
    assert.equal(res.body.estimate.recent.perChapter, 300, 'recent 那条路更厚（300）')
    assert.equal(res.body.estimate.recent.window, 149, '窗口比缺口大时，预估按整个缺口算')
    // ⚠️ "全部纳入"是 2 批而不是 1 批：首次补齐先走 30 章打底。预估漏掉打底就会少报一批。
    assert.equal(res.body.estimate.all.batches, 2, '首次补齐要先走 30 章打底')
    assert.equal(res.body.estimate.recent.batches, 2, '149 章 ÷ 每批 80 章（24000 ÷ 300）')
...
test('跳读闸：mode=recent 只记最近这一段，**绝不碰前面那几百章**', async () => {
  ...
  try {
    ...
    // ⚠️ 这里拿到的是 **600**，不是下限 300：20 章 × 300 = 6000，远小于 24000 的预算，
    // 均分额度算出来是 1200、被 `maxPerChapter` 封顶到 600 —— 也就是说**窗口窄的时候
    // 这一段直接拿满**。下限 300 真正起作用是在窗口很宽时（见下一条用例）。
    assert.equal(res.body.sampled.perChapter, 600, '窄窗口下均分额度被 maxPerChapter 封顶 = 这一段拿满')
...
test('跳读闸：默认窗口大于缺口时夹到缺口起点（不能算出 0 或负章号）', async () => {
  ...
  // 同时钉住"更厚 = 一批更少吃几章"：下限 300 → 24000 ÷ 300 = 80 章，装不下 149。
  ...
  try {
    ...
    assert.equal(res.body.sampled.perChapter, 300)
    assert.ok(res.body.sampled.chapters < 149, '每章下限 300，一趟装不下 149 章')
    assert.ok(res.body.sampled.chapters >= 60, '但也不该只吃个零头')
