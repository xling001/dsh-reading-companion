...
test('接线守卫：草稿待落盘清单、未选定原文标记、丢弃确认（v1.54）', () => {
  ...
  // 丢弃不可逆 → 走确认条。
  assert.ok(source.includes('discardConfirm'), '丢弃要有二次确认状态')
  assert.ok(!source.includes('onClick: discard }'), '丢弃不该再是一键执行')
  assert.ok(source.includes('setDiscardConfirm(true)'), '丢弃按钮应当只是"要求确认"')
})
test('接线守卫：三个选项卡 + 删除/恢复/彻底删除（v1.57）', () => {
  const here = dirname(fileURLToPath(import.meta.url))
  const source = readFileSync(join(here, '..', 'lib', 'client.js'), 'utf8')

  assert.ok(source.includes('笔记（${notesTotal}）'), '要有一个「笔记」选项卡')
  assert.ok(source.includes('草稿（${drafts.length}）'), '要有一个「草稿」选项卡')
  assert.ok(source.includes('回收站（${trashNotes.length}）'), '要有一个「回收站」选项卡')
